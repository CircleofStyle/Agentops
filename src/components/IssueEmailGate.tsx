...
type IssueEmailGateProps = {
  teaser: string;
  setupMinutes?: number;
  /** SEO shell override — hides default teaser block when public summary is shown above. */
  hideTeaser?: boolean;
  gateHeadline?: string;
  gateSubcopy?: string;
};

export function IssueEmailGate({
  teaser,
  setupMinutes,
  hideTeaser = false,
  gateHeadline,
  ...
  return (
    <div className="mt-10 space-y-8">
      ...
      <div className="rounded-xl border border-brand-500/30 bg-brand-500/5 p-6 sm:p-8">
        <h2 className="text-xl font-bold text-white">{gateHeadline ?? t.title}</h2>
        <p className="mt-2 text-slate-400">{gateSubcopy ?? t.subtitle}</p>
        <div className="mt-6">
          <SignupForm />
        </div>
      </div>
