...
import { IssueArticleSchema } from "@/components/IssueArticleSchema";
import { IssueEmailGate } from "@/components/IssueEmailGate";
import { IssueFaqAccordion } from "@/components/IssueFaqAccordion";
import { IssueMetadataBadges } from "@/components/IssueMetadataBadges";
import { IssueSeoLinksFooter } from "@/components/IssueSeoLinksFooter";
import { IssueSeoPublicSummary } from "@/components/IssueSeoPublicSummary";
...
import { getSeoShell } from "@/lib/content/seo-shell";
import { buildIssuePageMetadata } from "@/lib/seo-issue-metadata";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seoShell = await getSeoShell(slug, locale);

  return buildIssuePageMetadata(issue, locale, seoShell);
}
...
export default async function IssuePage({ params }: PageProps) {
  ...
  const summaryHtml = seoShell ? await markdownToHtml(seoShell.publicSummary) : null;

  return (
    <>
      {seoShell ? <FaqSchema items={seoShell.faqs} /> : null}
      <IssueArticleSchema issue={issue} locale={locale} />
      ...
      <article className="relative mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
        ...
        {summaryHtml ? <IssueSeoPublicSummary html={summaryHtml} /> : null}

        {seoShell ? (
          <IssueFaqAccordion items={seoShell.faqs} heading={dict.seoShell.faqHeading} />
        ) : null}
        ...
        {showFullBody && html ? (
          ...
          <IssueEmailGate
            teaser={teaser}
            setupMinutes={issue.frontmatter.setupMinutes}
            hideTeaser={Boolean(seoShell)}
            gateHeadline={seoShell?.gateHeadline}
            gateSubcopy={seoShell?.gateSubcopy}
          />
        )}

        {showFullBody && seoShell?.dripTeaser ? (
          <section className="mt-10 rounded-xl border border-slate-800 bg-slate-900/40 p-6 sm:p-8">
            <p className="text-slate-400">{seoShell.dripTeaser}</p>
            <div className="mt-6">
              <SignupForm />
            </div>
          </section>
        ...
        {seoShell ? (
          <IssueSeoLinksFooter
            slug={seoShell.slug}
            locale={locale}
            heading={dict.seoShell.relatedLinksHeading}
          />
        ) : null}
      </article>
    </main>
